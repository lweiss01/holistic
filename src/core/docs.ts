...
function withProvenance(content: string): string {
  return `<!-- ${GENERATED_MARKER}: DO NOT EDIT THIS FILE DIRECTLY -->\n<!-- Holistic version: ${VERSION} -->\n\n${content}`;
}

/**
 * Whether a document is hand-authored and must not be overwritten.
 *
 * Adapters are documented as an extension point: AGENTS.md tells contributors
 * to register an agent by adding an adapter file rather than editing source.
 * That promise was false, because every adapter was regenerated from
 * ADAPTER_PROFILES on each run and hand-edits were silently reverted.
 *
 * A file is treated as user-authored when it carries an explicit
 * HOLISTIC-USER-EDITED marker, or when it has lost the generated marker
 * entirely. Untouched generated files keep receiving template updates.
 */
export function isUserAuthoredDoc(filePath: string): boolean {
  ...
  try {
    ...
    if (content.includes(USER_EDITED_MARKER)) {
      return true;
    }
    return !content.includes(GENERATED_MARKER);
  } catch {
    // Unreadable: treat as user-authored so we never clobber what we cannot see.
    return true;
  }
}

/**
 * Re-stamp a doc that lost its HOLISTIC-GENERATED marker so it becomes
 * refreshable again. Opt-in only: losing the marker is indistinguishable from a
 * deliberate customisation, so this is never called automatically.
 *
 * A doc carrying an explicit HOLISTIC-USER-EDITED marker is never touched --
 * that marker is the documented way to opt out of refreshes permanently.
 * Returns true when the file was re-stamped.
 */
export function restampGeneratedMarker(filePath: string): boolean {
  if (!fs.existsSync(filePath)) {
    return false;
  }
  let content: string;
  ...
  if (content.includes(USER_EDITED_MARKER) || content.includes(GENERATED_MARKER)) {
    return false;
  }
  fs.writeFileSync(filePath, `<!-- ${GENERATED_MARKER}: DO NOT EDIT THIS FILE DIRECTLY -->\n\n${content}`, "utf8");
  return true;
}

/** Whether a doc still carries the marker that makes it eligible for refresh. */
export function hasGeneratedMarker(filePath: string): boolean {
  try {
...
function renderHolisticMd(state: HolisticState, options: { safeMode?: boolean } = {}): string {
  ...
  if (options.safeMode) {
    return `# HOLISTIC (Safe Mode)

<!-- HOLISTIC-GENERATED: MINIMAL INSTRUCTIONS -->

## Start Here

This repo uses Holistic for cross-agent handoffs. The source of truth is the repo itself.

${resumeFallbackNote}

Review the handoff docs in ${state.docIndex.contextDir} before starting.
`;
  }
...
refreshed from the built-in templates on every run. To customise one and keep
your changes, either delete that comment or add a line containing
\`${USER_EDITED_MARKER}\`. Holistic then leaves the file alone permanently.

A new adapter for an agent Holistic does not ship is never touched.
`;
}
...
export function writeDerivedDocs(paths: RuntimePaths, state: HolisticState, options: WriteDerivedDocsOptions = {}): void {
  ...
  writeDerivedDoc(paths, paths.masterDoc, withProvenance(renderHolisticMd(state, { safeMode })), mode);
  writeDerivedDoc(paths, paths.agentsDoc, withProvenance(renderAgentsMd(state)), mode);
  writeDerivedDoc(paths, paths.currentPlanDoc, withProvenance(renderCurrentPlan(state)), mode);
  writeDerivedDoc(paths, paths.protocolDoc, withProvenance(renderProtocol(state)), mode);
  writeDerivedDoc(paths, paths.historyDoc, withProvenance(renderProjectHistory(paths, state)), mode);
  writeDerivedDoc(paths, paths.regressionDoc, withProvenance(renderRegressionWatch(paths, state)), mode);
  writeDerivedDoc(paths, paths.zeroTouchDoc, withProvenance(renderZeroTouchDoc(state)), mode);
  ...
  for (const profile of ADAPTER_PROFILES) {
    ...
    // subsequent run, or the documented extension path does not work.
    if (isUserAuthoredDoc(adapterPath)) {
      continue;
    }
    writeDerivedDoc(paths, adapterPath, withProvenance(renderAdapter(state, profile)), mode);
  }

  if (paths.rootClaudeDoc) {
    writeDerivedDoc(paths, paths.rootClaudeDoc, withProvenance(renderRootAgentDoc("Claude/Cowork", "claude", true)), mode);
  }
  if (paths.rootGeminiDoc) {
    writeDerivedDoc(paths, paths.rootGeminiDoc, withProvenance(renderRootAgentDoc("Gemini", "gemini", false)), mode);
  }
  if (paths.rootHistoryDoc) {
    writeDerivedDoc(paths, paths.rootHistoryDoc, withProvenance(renderRootHistoryMd(paths, state)), mode);
  }
  if (paths.rootCursorRulesDoc) {
    writeDerivedDoc(paths, paths.rootCursorRulesDoc, withProvenance(renderCursorRules(state)), mode);
  }
  if (paths.rootWindsurfRulesDoc) {
    writeDerivedDoc(paths, paths.rootWindsurfRulesDoc, withProvenance(renderWindsurfRules(state)), mode);
  }
  if (paths.rootCopilotInstructionsDoc) {
    writeDerivedDoc(paths, paths.rootCopilotInstructionsDoc, withProvenance(renderCopilotInstructions(state)), mode);
  }
}

...
function withProvenance(content: string): string {
  return `<!-- HOLISTIC-GENERATED: DO NOT EDIT THIS FILE DIRECTLY -->\n<!-- Holistic version: ${VERSION} -->\n\n${content}`;
}
...
export function writeDerivedDocs(paths: RuntimePaths, state: HolisticState, options: WriteDerivedDocsOptions = {}): void {
  ...
  for (const profile of ADAPTER_PROFILES) {
    ...
    writeDerivedDoc(paths, path.join(paths.adaptersDir, fileName), withProvenance(renderAdapter(state, profile)), mode);
  }

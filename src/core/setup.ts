...
function renderAndonHookPs1(): string {
  return [
    ...
    "    $source = \"collector\"",
    "    $payload = @{ stopReason = $hookData.stop_reason; sourceType = $sourceType; sourceId = \"andon-hook\"; transport = \"http_events\" }",
    "}",
    "",
...
function renderAndonHookSh(): string {
  return [
    ...
    "    payload=\"{\\\"stopReason\\\":$stop_reason_escaped,\\\"sourceType\\\":\\\"$source_type\\\",\\\"sourceId\\\":\\\"andon-hook\\\",\\\"transport\\\":\\\"http_events\\\"}\"",
    "fi",
    "",

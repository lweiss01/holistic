...
const tests: Array<{ name: string; run: () => void | Promise<void> }> = [
  ...
  {
    name: "Andon fleet hides legacy-only sessions when runtime source is missing",
    run: async () => {
      ...
      try {
        ...
        assert.equal(payload.sessions.some((item) => item.session.id === "legacy-visible-session"), false);

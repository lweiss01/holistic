  ...
  switch (input.status.status) {
    ...
    case "at_risk":
      return {
        ...
        description:
          input.holisticContext && input.holisticContext.acceptedApproaches.length > 0
    ...
    case "awaiting_review":
      return {
        ...
        description:
          "Inspect the latest summary, changed files, and tests to decide whether to accept the work or request follow-up."
      };
    case "queued":
      return {
        urgency: "low",
    ...
    case "parked":
      return {
        ...
        description:
          "The session looks inactive. Resume it if the work still matters, or leave it parked if it is intentionally paused."
      };
    case "running":
    default:
      return {
        ...
        description:
          "No intervention is needed yet. The agent is active and the current signals look healthy."
      };
  }
}
